...
export default function CompanyInterviews() {
  ...
  const handleCancelClick = async (id) => {
    const reason = window.prompt(
      "Enter cancellation notes/reason for the candidate:",
    );
  ...
  const handleViewCoach = async (interviewId, applicationId) => {
    ...
    try {
      ...
      toast.error(
        error.message ||
          "Preparation guide is still generating or failed to load",
      );
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6 max-w-7xl animate-settle">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-(--color-brand-blue)">
            Company Interviews
          </h1>
          <p className="text-slate-500 text-sm">
            Review, reschedule, and manage scheduled interviews for your company
            applications.
          </p>
        </div>
      </div>
      ...
      {isCoachModalOpen && (
        <Modal
          ...
          onOpenChange={(open) => !open && setIsCoachModalOpen(false)}>
            ...
            <div className="mt-4">
              {isLoadingCoach ? (
                <div className="p-12 text-center text-slate-500">
                  Evaluating candidate skills and rendering preparation tips...
                </div>
              ) : (
